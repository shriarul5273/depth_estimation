"""
OmniVGGT — self-contained single-file model implementation.

All architecture components (DINOv2 ViT layers, alternating-attention aggregator,
DPT head, camera head, pose utilities) are inlined here so no external omnivggt
package is required.

Loads weights from HuggingFace Hub (Livioni/OmniVGGT).
"""
...
from .configuration_omnivggt import OmniVGGTConfig
...
class PoseEmbedding(nn.Module):
    def __init__(self, hidden_dim=512, out_dim=1024):
        super().__init__()
...
# ═══════════════════════════════════════════════════════════════════════════════
# §4  Utilities (rotation, pose encoding, geometry)
# ═══════════════════════════════════════════════════════════════════════════════
...
def closed_form_inverse_se3(se3, R=None, T=None):
    ...
    if is_numpy:
        ...
        inv = torch.eye(4, 4)[None].repeat(len(R), 1, 1).to(R.dtype).to(R.device)
    inv[:, :3, :3] = R_t
    inv[:, :3, 3:] = top_right
    return inv
...
class Aggregator(nn.Module):
    def __init__(self, img_size=518, patch_size=14, embed_dim=1024, depth=24,
                 ...
                 qk_norm=True, rope_freq=100, init_values=0.01, enable_checkpoint=True):
        super().__init__()
        if aa_order is None:
    ...
    def _process_frame_attention(self, tokens, B, S, P, C, frame_idx, pos=None):
        ...
        for _ in range(self.aa_block_size):
            ...
            if self.use_checkpoint and self.training:
                tokens = checkpoint(lambda inp, p: blk(inp, pos=p), tokens, pos, use_reentrant=False)
            else:
...
class ZeroAggregator(Aggregator):
    def __init__(self, img_size=518, patch_size=14, embed_dim=1024, depth=24,
                 num_heads=16, mlp_ratio=4, num_register_tokens=4, block_fn=Block,
                 pose_hidden_dim=9, cam_drop_prob=0.1, depth_drop_prob=0.1,
                 qkv_bias=True, proj_bias=True, ffn_bias=True,
                 patch_embed="dinov2_vitl14_reg", aa_order=None,
                 aa_block_size=1, qk_norm=True, rope_freq=100, init_values=0.01,
                 enable_checkpoint=True):
        super().__init__(img_size, patch_size, embed_dim, depth, num_heads, mlp_ratio,
                         num_register_tokens, block_fn, qkv_bias, proj_bias, ffn_bias,
                         patch_embed, aa_order, aa_block_size, qk_norm, rope_freq,
                         init_values, enable_checkpoint)
        ...
        for _ in range(self.num_groups):
            self.pose_embeddings.append(nn.Linear(pose_hidden_dim, embed_dim))
            cam_adapt = nn.Linear(embed_dim, embed_dim, bias=True)
            nn.init.zeros_(cam_adapt.weight); nn.init.zeros_(cam_adapt.bias)
            self.camera_adapters.append(cam_adapt)
        self.depth_patch_embed = PatchEmbed(img_size=img_size, patch_size=patch_size,
    ...
    def normalize_extrinsics(self, extrinsics):
        ...
        ext_h = torch.cat([extrinsics, torch.zeros((B, S, 1, 4), device=device)], dim=-2)
        ext_h[:, :, -1, -1] = 1.0
        first_inv = closed_form_inverse_se3(ext_h[:, 0])
        new_ext = torch.matmul(ext_h, first_inv.unsqueeze(1))
        if S > 1:
            cam_centers = new_ext[:, :, :3, 3]
            rel_distances = torch.norm(cam_centers - cam_centers[:, 0:1], dim=-1)[:, 1:]
            scale = rel_distances.mean(dim=1, keepdim=True).clamp(min=1e-6)
            new_ext[:, :, :3, 3] /= scale.unsqueeze(-1)
        return new_ext[:, :, :3]

    def normalize_depth(self, depth, mask, eps=1e-8):
        B, V, H, W, _ = depth.shape
        depth_sq = depth.squeeze(-1)
        norm = torch.zeros_like(depth_sq)
        for b in range(B):
            valid = depth_sq[b][mask[b] > 0]
            if valid.numel() == 0:
                continue
            mean = valid.mean()
            norm[b] = depth_sq[b] / (mean + eps) * mask[b]
        return norm.unsqueeze(-1)

    def inference(self, images, extrinsics=None, intrinsics=None, depth=None, mask=None,
                  depth_gt_index=None, camera_gt_index=None):
        if depth_gt_index is None:
            depth_gt_index = []
        if camera_gt_index is None:
            camera_gt_index = []
        if extrinsics is None:
            B, S, _, H, W = images.shape
            extrinsics = torch.zeros(B, S, 3, 4, device=images.device)
            intrinsics = torch.eye(3, device=images.device).unsqueeze(0).unsqueeze(0).expand(B, S, -1, -1)
        if depth is None:
            B, S, _, H, W = images.shape
            depth = torch.zeros(B, S, H, W, 1, device=images.device)
            mask = torch.zeros(B, S, H, W, device=images.device)
        ...
        if len(camera_gt_index) != 0:
            cam_idx_t = torch.tensor(camera_gt_index, device=images.device)
            ext_sel = torch.index_select(extrinsics, dim=1, index=cam_idx_t)
            int_sel = torch.index_select(intrinsics, dim=1, index=cam_idx_t)
            ext_norm = self.normalize_extrinsics(ext_sel)
            pose_enc = extri_intri_to_pose_encoding(ext_norm, int_sel, (H, W))
            gt_cam_tok = self.pose_embeddings[0](pose_enc).view(B * len(camera_gt_index), C).unsqueeze(1)
            cam_full = torch.zeros(K, 1, C, device=images.device, dtype=camera_token.dtype)
            cam_rows = (torch.arange(B, device=images.device).unsqueeze(1) * S +
                        cam_idx_t.unsqueeze(0)).reshape(-1)
            cam_full[cam_rows] = gt_cam_tok.to(dtype=camera_token.dtype)
            gt_cam_tok = cam_full
            pose_encoding_val = pose_enc
        else:
            pose_encoding_val = None
            gt_cam_tok = torch.zeros(K, 1, C, device=images.device, dtype=camera_token.dtype)

        if len(depth_gt_index) != 0:
            idx_t = torch.tensor(depth_gt_index, device=images.device)
            d_sel = torch.index_select(depth, dim=1, index=idx_t)
            m_sel = torch.index_select(mask, dim=1, index=idx_t)
            d_norm = self.normalize_depth(d_sel, m_sel)
            d_norm = d_norm.view(B * len(depth_gt_index), 1, H, W)
            m_sel_flat = m_sel.view(B * len(depth_gt_index), 1, H, W)
            dmaps = self._match_dtype(torch.cat([d_norm, m_sel_flat], dim=1),
                                      self.depth_patch_embed.proj.weight)
            gt_depth_tok = self.depth_patch_embed(dmaps)
            depth_full = self.depth_placeholder.expand(K, P, C).clone()
            rows = (torch.arange(B, device=images.device).unsqueeze(1) * S +
                    idx_t.unsqueeze(0)).reshape(-1)
            depth_full[rows] = gt_depth_tok.to(dtype=patch_tokens.dtype)
            gt_depth_tok = depth_full
        else:
            gt_depth_tok = self.depth_placeholder.expand(K, P, C)

        camera_token = camera_token + self.camera_adapters[0](gt_cam_tok)
        patch_tokens = patch_tokens + gt_depth_tok
        ...
        for index in range(self.aa_block_num):
            for attn_type in self.aa_order:
                if attn_type == "frame":
                    tokens, frame_idx, frame_ints = self._process_frame_attention_zero(
                        tokens, B, S, P, C, frame_idx, pos=pos, index=index + 1,
                        camera_gt_index=camera_gt_index, pose_encoding=pose_encoding_val,
                        register_shape=register_token.shape, P_old=P_old)
    ...
    def _process_frame_attention_zero(self, tokens, B, S, P, C, frame_idx, pos=None,
                                       index=None, camera_gt_index=None, pose_encoding=None,
                                       register_shape=None, P_old=None):
        if tokens.shape != (B * S, P, C):
            tokens = tokens.view(B, S, P, C).view(B * S, P, C)
        if pos is not None and pos.shape != (B * S, P, 2):
            pos = pos.view(B, S, P, 2).view(B * S, P, 2)
        intermediates = []
        K = tokens.shape[0]
        reg_tok = torch.zeros(register_shape, device=tokens.device, dtype=tokens.dtype).expand(B * S, -1, -1)
        if camera_gt_index and len(camera_gt_index) != 0:
            cam_idx_t = torch.tensor(camera_gt_index, device=tokens.device)
            gt_ct = self.pose_embeddings[index](pose_encoding).view(B * len(camera_gt_index), C).unsqueeze(1)
            cam_full = torch.zeros(K, 1, C, device=tokens.device, dtype=gt_ct.dtype)
            cam_rows = (torch.arange(B, device=tokens.device).unsqueeze(1) * S +
                        cam_idx_t.unsqueeze(0)).reshape(-1)
            cam_full[cam_rows] = gt_ct.to(dtype=cam_full.dtype)
        else:
            cam_full = torch.zeros(K, 1, C, device=tokens.device, dtype=tokens.dtype)
        depth_inj = torch.zeros(K, P_old, C, device=tokens.device, dtype=tokens.dtype)
        cam_inj = self.camera_adapters[index](cam_full)
        injection = torch.cat([cam_inj, reg_tok, depth_inj], dim=1)
        for _ in range(self.aa_block_size):
            blk = self.frame_blocks[frame_idx]
            if self.use_checkpoint and self.training:
                tokens = checkpoint(lambda inp, p: blk(inp, pos=p), tokens, pos, use_reentrant=False)
...
# ═══════════════════════════════════════════════════════════════════════════════
# §7  OmniVGGT main model
# ═══════════════════════════════════════════════════════════════════════════════

class _OmniVGGT(nn.Module):
    """Internal OmniVGGT architecture (no HuggingFace mixin)."""

    def __init__(self, img_size=518, patch_size=14, embed_dim=1024,
                 cam_drop_prob=0.1, depth_drop_prob=0.1,
                 enable_camera=True, enable_depth=True, enable_point=True):
        super().__init__()
        self.aggregator = ZeroAggregator(
    ...
    def inference(self, images, extrinsics=None, intrinsics=None, depth=None, mask=None,
        ...
        agg_tokens, patch_start_idx = self.aggregator.inference(
        ...
        with torch.amp.autocast('cuda', enabled=False):
            if self.camera_head is not None:
                pose_enc_list = self.camera_head(agg_tokens)
                predictions["pose_enc"] = pose_enc_list[-1]
                predictions["pose_enc_list"] = pose_enc_list
            if self.depth_head is not None:
                depth_pred, depth_conf = self.depth_head(agg_tokens, images, patch_start_idx)
                predictions["depth"] = depth_pred
                predictions["depth_conf"] = depth_conf
            if self.point_head is not None:
                pts3d, pts3d_conf = self.point_head(agg_tokens, images, patch_start_idx)
...
class OmniVGGTModel(BaseDepthModel):
    """OmniVGGT monocular depth estimation wrapper.

    Adapts the multi-frame OmniVGGT architecture for single-image depth estimation
    ...
    config_class = OmniVGGTConfig

    def __init__(self, config: OmniVGGTConfig):
    ...
    def forward(self, pixel_values: torch.Tensor) -> torch.Tensor:
        ...
        preds = self._net.inference(images)
    ...
    @classmethod
    def _load_pretrained_weights(cls, model_id: str, device: str = "cpu", **kwargs) -> "OmniVGGTModel":
        from safetensors.torch import load_file
        from huggingface_hub import hf_hub_download

        config = OmniVGGTConfig.from_variant(model_id)
        torch_device = torch.device(device)

        net = _OmniVGGT(img_size=config.img_size, patch_size=config.patch_size,
                        embed_dim=config.embed_dim)

        ckpt_path = hf_hub_download(repo_id=config.hub_repo_id, filename="OmniVGGT.safetensors")
    ...
    def _backbone_module(self):
        """Return the ZeroAggregator (frame + global attention blocks) as the backbone."""
    ...
    def unfreeze_top_k_backbone_layers(self, k: int) -> None:
        """Unfreeze the last k frame_blocks and last k global_blocks of the ZeroAggregator."""
