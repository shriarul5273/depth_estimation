...
def drop_add_residual_stochastic_depth(
    x, residual_func, sample_drop_ratio=0.0, pos=None
):
    b, n, d = x.shape
    ...
    x_subset = x[brange]
    residual = (
        residual_func(x_subset, pos=pos[brange])
        if pos is not None
        else residual_func(x_subset)
    )
    x_flat = x.flatten(1)
    residual = residual.flatten(1)
    scale = b / sample_subset_size
    return torch.index_add(
        x_flat, 0, brange, residual.to(dtype=x.dtype), alpha=scale
    ).view_as(x)
    ...
    def forward(self, x: Tensor, pos=None) -> Tensor:
        ...
        if self.training and self.sample_drop_ratio > 0.1:
            x = drop_add_residual_stochastic_depth(
                x,
                pos=pos,
                residual_func=attn_res,
                sample_drop_ratio=self.sample_drop_ratio,
            )
            x = drop_add_residual_stochastic_depth(
                x, residual_func=ffn_res, sample_drop_ratio=self.sample_drop_ratio
            )
    ...
    def _process_frame_attention(self, tokens, B, S, P, C, frame_idx, pos=None):
        ...
        for _ in range(self.aa_block_size):
            ...
            if self.use_checkpoint and self.training:
                tokens = checkpoint(
                    lambda inp, p: blk(inp, pos=p), tokens, pos, use_reentrant=False
                )
            else:
...
class ZeroAggregator(Aggregator):
    ...
    def normalize_extrinsics(self, extrinsics):
        ...
        ext_h = torch.cat(
            [extrinsics, torch.zeros((B, S, 1, 4), device=device)], dim=-2
        )
        ext_h[:, :, -1, -1] = 1.0
    ...
    def inference(
        ...
        if extrinsics is None:
            ...
            intrinsics = (
                torch.eye(3, device=images.device)
                .unsqueeze(0)
                .unsqueeze(0)
                .expand(B, S, -1, -1)
            )
        ...
        if len(camera_gt_index) != 0:
            ...
            gt_cam_tok = (
                self.pose_embeddings[0](pose_enc)
                .view(B * len(camera_gt_index), C)
                .unsqueeze(1)
            )
            cam_full = torch.zeros(
                K, 1, C, device=images.device, dtype=camera_token.dtype
            )
            cam_rows = (
                torch.arange(B, device=images.device).unsqueeze(1) * S
                + cam_idx_t.unsqueeze(0)
            ).reshape(-1)
        ...
        if len(depth_gt_index) != 0:
            ...
            dmaps = self._match_dtype(
                torch.cat([d_norm, m_sel_flat], dim=1),
            ...
            for attn_type in self.aa_order:
                if attn_type == "frame":
                    tokens, frame_idx, frame_ints = self._process_frame_attention_zero(
                        ...
                        pose_encoding=pose_encoding_val,
    ...
    def _process_frame_attention_zero(
        self,
        tokens,
        B,
        S,
...
def activate_pose(
    ...
    return torch.cat(
        [base_act(T, trans_act), base_act(quat, quat_act), base_act(fl, fl_act)], dim=-1
    )
    ...
    def forward(self, aggregated_tokens_list, num_iterations=4):
        ...
        for _ in range(num_iterations):
            ...
            pred_pose_enc_list.append(
                activate_pose(pred_pose_enc, self.trans_act, self.quat_act, self.fl_act)
            )
...
def _make_scratch(in_shape, out_shape, groups=1, expand=False):
    ...
    scratch.layer1_rn = nn.Conv2d(
        in_shape[0], out_shape1, 3, 1, 1, bias=False, groups=groups
    )
    scratch.layer2_rn = nn.Conv2d(
        in_shape[1], out_shape2, 3, 1, 1, bias=False, groups=groups
    )
    scratch.layer3_rn = nn.Conv2d(
        in_shape[2], out_shape3, 3, 1, 1, bias=False, groups=groups
    )
    if len(in_shape) >= 4:
        scratch.layer4_rn = nn.Conv2d(
            in_shape[3], out_shape4, 3, 1, 1, bias=False, groups=groups
        )
    return scratch
...
def custom_interpolate(
    ...
    if size[0] * size[1] * x.shape[0] * x.shape[1] > INT_MAX:
        chunks = torch.chunk(
            x,
            chunks=(size[0] * size[1] * x.shape[0] * x.shape[1] // INT_MAX) + 1,
            dim=0,
        )
        return torch.cat(
            [
    ...
    def __init__(
        ...
        if has_residual:
            self.resConfUnit1 = ResidualConvUnit(
                features, activation, bn, groups=groups
            )
...
class DPTHead(nn.Module):
    ...
    def forward(
        self, aggregated_tokens_list, images, patch_start_idx, frames_chunk_size=8
    ):
        B, S, _, H, W = images.shape
        ...
        for fs in range(0, S, frames_chunk_size):
            ...
            preds_c, conf_c = self._forward_impl(
                aggregated_tokens_list, images, patch_start_idx, fs, fe
            )
    ...
    def _apply_pos_embed(self, x, W, H, ratio=0.1):
        ...
        pos_embed = create_uv_grid(
            pw, ph, aspect_ratio=W / H, dtype=x.dtype, device=x.device
        )
        ...
        with torch.amp.autocast("cuda", enabled=False):
            ...
            if self.depth_head is not None:
                depth_pred, depth_conf = self.depth_head(
                    agg_tokens, images, patch_start_idx
                )
...
class OmniVGGTModel(BaseDepthModel):
    ...
    def _load_pretrained_weights(
        cls, model_id: str, device: str = "cpu", **kwargs
    ) -> "OmniVGGTModel":
        ...
        ckpt_path = hf_hub_download(
            repo_id=config.hub_repo_id, filename="OmniVGGT.safetensors"
        )
